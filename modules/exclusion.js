...
export function generate_exclusion_puzzle(size, score_lower_limit = 0, holes_count = undefined) {
    ...
    while (marks_added < num_marks) {
        ...
        if (
            ...
            const result = solve(board.map(r => r.map(cell => cell === 0 ? [...Array(size)].map((_, n) => n + 1) : cell)), size, isValid_exclusion, true);
...
export function apply_exclusion_marks(board, size) {
    ...
    for (const mark of marks) {
        ...
        const excludedNum = parseInt(value);
        ...
        const colMark = Math.round((left - gridOffsetLeft + 15) / cellWidth);
        const rowMark = Math.round((top - gridOffsetTop + 15) / cellHeight);
        ...
        for (const [r, c] of positions) {
            if (r >= 0 && r < size && c >= 0 && c < size) {
                ...
                if (Array.isArray(board[r][c])) {
                    board[r][c] = board[r][c].filter(n => n !== excludedNum);
                }
            }
        }
    }
...
/**
 * 排除数独有效性检测函数
 * 除常规区域外，额外判断所有排除标记交点四格不能填入被排除数字
 */
export function isValid_exclusion(board, size, row, col, num) {
    // 1. 常规区域判断（与普通数独一致）
    ...
    for (const mark of marks) {
        ...
        for (const [r, c] of positions) {
            if (r === row && c === col && num === excludedNum) {
                return false;
            }
        }
    }
