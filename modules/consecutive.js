...
export function create_consecutive_sudoku(size) {
    ...
    enable_consecutive_mark_mode(size);
    ...
    if (extraButtons) {
        ...
        add_Extra_Button('自动出题', state.create_mode_specific_generate_handler?.((score_lower_limit, holes_count) => generate_consecutive_puzzle(size, score_lower_limit, holes_count)) || (() => generate_consecutive_puzzle(size)), '#2196F3');
    }

    reset_consecutive_highlights(container);
}

function auto_mark_consecutive() {
    ...
    if (added > 0) {
        show_result(`已自动标记${added}个连续关系`, 'success');
    } else {
        show_result('未找到需要标记的连续关系', 'info');
    }
}

export function generate_consecutive_puzzle(size, score_lower_limit = 0, holes_count = undefined) {
...
function enable_consecutive_mark_mode(size) {
    ...
    grid.addEventListener('click', (event) => {
        ...
        if (grid._consecutiveClickTimer) {
            clearTimeout(grid._consecutiveClickTimer);
            grid._consecutiveClickTimer = null;
        }
    ...
    grid.addEventListener('dblclick', (event) => {
        ...
        if (dist_to_vertical < dist_to_horizontal && dist_to_vertical < threshold && col < size - 1) {
            create_or_update_consecutive_mark(container, size, row, col, row, col + 1, 'W', true);
        } else if (dist_to_horizontal <= dist_to_vertical && dist_to_horizontal < threshold && row < size - 1) {
            create_or_update_consecutive_mark(container, size, row, col, row + 1, col, 'W', true);
        }
    });
}
...
function populate_all_consecutive_marks(container, board, size) {
    let count = 0;
    for (let row = 0; row < size; row++) {
        for (let col = 0; col < size; col++) {
            if (col < size - 1) {
...
function create_or_update_consecutive_mark(container, size, row1, col1, row2, col2, type = 'W', focus_input = false) {
    ...
    if (focus_input) {
        mark.classList.add('consecutive-mark-highlight');
...
function get_consecutive_container() {
