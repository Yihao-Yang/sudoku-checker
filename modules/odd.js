...
export function create_odd_sudoku(size) {
    ...
    add_Extra_Button('自动出题', state.create_mode_specific_generate_handler?.((score_lower_limit, holes_count) => generate_odd_puzzle(size, score_lower_limit, holes_count)) || (() => generate_odd_puzzle(size)), '#2196F3');
...
export function generate_odd_puzzle(size, score_lower_limit = 0, holes_count = undefined) {
    ...
    // 按宫大小调整奇数标记数量
    if (size === 4) {
        odd_marks_count = Math.floor(Math.random() * (4 - 2 + 1)) + 2; // 2-4
    } else if (size === 6) {
    ...
    // 随机生成奇数标记位置（不重复，带对称，每次添加后判断有解）
    const positions_set = new Set();
    let marks_added = 0;
    let try_limit = 1000; // 防止死循环
    while (marks_added < odd_marks_count && try_limit-- > 0) {
        ...
        add_odd_mark_to_cell(cell1);
        if (row !== sym[0] || col !== sym[1]) add_odd_mark_to_cell(cell2);
        ...
        const result = solve(board.map(r => r.map(cell => cell === 0 ? [...Array(size)].map((_, n) => n + 1) : cell)), size, is_valid_odd, true);
        ...
        if (result.solution_count === 0 || result.solution_count === -2) {
            ...
            // 无解，撤销标记
            if (cell1) {
                const mark1 = cell1.querySelector('.odd-mark');
                if (mark1) mark1.remove();
            }
            if (cell2 && (row !== sym[0] || col !== sym[1])) {
                const mark2 = cell2.querySelector('.odd-mark');
    ...
    function add_odd_mark_to_cell(cell) {
        if (!cell || cell.querySelector('.odd-mark')) return;
        const newMark = document.createElement('div');
        newMark.className = 'odd-mark';
        newMark.style.position = 'absolute';
        newMark.style.left = '50%';
    ...
    for (const cell of cells) {
        if (cell.querySelector('.odd-mark')) {
            ...
            if (Array.isArray(board[row][col])) {
                board[row][col] = board[row][col].filter(n => n % 2 === 1);
            }
        }
    }
}
...
// 奇数数独有效性检测函数
export function is_valid_odd(board, size, row, col, num) {
    // 1. 常规区域判断（与普通数独一致）
    const mode = state.current_mode || 'odd';
    ...
    if (grid) {
        ...
        if (cell && cell.querySelector('.odd-mark')) {
            // 有奇数标记的格子只能填奇数
