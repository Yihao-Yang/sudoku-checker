...
// 同位数独主入口
export function create_anti_king_sudoku(size) {
    set_current_mode('anti_king');
    show_result(`当前模式为无缘数独`);
    log_process('', true);
    log_process('规则：');
    log_process('对角相邻位置（进一拐一）数字不重复');
    log_process('');
    log_process('技巧：');
    ...
    // 可添加唯一性验证等按钮
    add_Extra_Button('无缘', () => {create_anti_king_sudoku(size)}, '#2196F3');
}

/**
 * Anti-King 有效性检测函数
 * 除标准数独规则外，额外要求每个格子与其斜对角（王步）格子不能重复
 */
export function is_valid_anti_king(board, size, row, col, num) {
    // 标准数独区域判定
    const mode = 'classic';
