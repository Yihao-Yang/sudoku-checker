...
export function check_candidates_uniqueness() {
    ...
    function isValid(row, col, num) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            if ((typeof rowCell === 'number' && rowCell === num) || 
                (Array.isArray(rowCell) && rowCell.includes(num) && rowCell.length === 1)) {
                return false;
            }
            
            // 检查列
            const colCell = board[i][col];
            if ((typeof colCell === 'number' && colCell === num) || 
                (Array.isArray(colCell) && colCell.includes(num) && colCell.length === 1)) {
                return false;
            }
        }
        ...
        for (let r = startRow; r < startRow + boxSize[0]; r++) {
            for (let c = startCol; c < startCol + boxSize[1]; c++) {
                const boxCell = board[r][c];
                if ((typeof boxCell === 'number' && boxCell === num) || 
                    (Array.isArray(boxCell) && boxCell.includes(num) && boxCell.length === 1)) {
                    return false;
                }
            }
        }
