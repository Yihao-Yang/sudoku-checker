...
export function is_valid_X_sums(board, size, row, col, num) {
    ...
    // 行：左提示
    const left_cell = board[row][0];
    if ((typeof left_cell === 'number' && left_cell > 0) || col === 0) {
        let x = (col === 0) ? num : left_cell;

        const temp_board = board.map((r, i) => r.map((c, j) => (i === row && j === col ? num : c)));
        const row_values = temp_board[row].slice(0, x);

        if (!row_values.some(val => typeof val !== 'number' || val <= 0) && col < x) {
            const clue = readClue(row + 1, 0);
            if (typeof clue === 'number' && clue > 0) {
                const sum = row_values.reduce((acc, val) => acc + val, 0);
    ...
    // 行：右提示
    const right_cell = board[row][size - 1];
    if ((typeof right_cell === 'number' && right_cell > 0) || col === size - 1) {
        let x = (col === size - 1) ? num : right_cell;

        const temp_board = board.map((r, i) => r.map((c, j) => (i === row && j === col ? num : c)));
        const row_values = temp_board[row].slice(size - x);

        if (!row_values.some(val => typeof val !== 'number' || val <= 0) && col >= size - x) {
            const clue = readClue(row + 1, size + 1);
            if (typeof clue === 'number' && clue > 0) {
                const sum = row_values.reduce((acc, val) => acc + val, 0);
                if (sum !== clue) return false;
            }
    ...
    // 列：上提示
    const top_cell = board[0][col];
    if ((typeof top_cell === 'number' && top_cell > 0) || row === 0) {
        let x = (row === 0) ? num : top_cell;

        const temp_board = board.map((r, i) => r.map((c, j) => (i === row && j === col ? num : c)));
        const col_values = temp_board.slice(0, x).map(r => r[col]);

        if (!col_values.some(val => typeof val !== 'number' || val <= 0) && row < x) {
            const clue = readClue(0, col + 1);
            if (typeof clue === 'number' && clue > 0) {
                const sum = col_values.reduce((acc, val) => acc + val, 0);
                if (sum !== clue) return false;
            }
        }
    }
    ...
    // 列：下提示
    const bottom_cell = board[size - 1][col];
    if ((typeof bottom_cell === 'number' && bottom_cell > 0) || row === size - 1) {
        let x = (row === size - 1) ? num : bottom_cell;

        const temp_board = board.map((r, i) => r.map((c, j) => (i === row && j === col ? num : c)));
        const col_values = temp_board.slice(size - x).map(r => r[col]);

        if (!col_values.some(val => typeof val !== 'number' || val <= 0) && row >= size - x) {
            const clue = readClue(size + 1, col + 1);
            if (typeof clue === 'number' && clue > 0) {
                const sum = col_values.reduce((acc, val) => acc + val, 0);
                if (sum !== clue) return false;
            }
