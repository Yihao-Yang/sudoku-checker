...
export function generate_quadruple_puzzle(size, score_lower_limit = 0, holes_count = undefined) {
    ...
    const remove_quadruple_marks_by_keys = (keys = []) => {
        ...
        state.marks_board = state.marks_board.filter((m) => {
            ...
            return !keySet.has(get_quadruple_mark_key(m.r, m.c));
        });
    };
    ...
    setTimeout(() => {
        while (try_count < MAX_TRY && marks_added < MAX_MARKS && !unique_found) {
            ...
            if (
                ...
                has_quadruple_mark(row, col) ||
                has_quadruple_mark(sym_row, sym_col)
            ) {
                continue;
            }
            ...
            upsert_quadruple_mark(row, col, mainDigits.join(''));
            addedKeys.push(get_quadruple_mark_key(row, col));
            ...
            if (!symmetric_is_same) {
                ...
                if (!symDigits) {
                    remove_quadruple_marks_by_keys(addedKeys);
                    continue;
                }

                upsert_quadruple_mark(sym_row, sym_col, symDigits.join(''));
                addedKeys.push(get_quadruple_mark_key(sym_row, sym_col));
            }
        ...
        render_quadruple_marks_from_state(size, container);
    ...
    function group_mark_keys_by_symmetry(size, symmetry) {
        ...
        const keySet = new Set(marks.map((m) => get_quadruple_mark_key(m.r, m.c)));

        for (const mark of marks) {
            const key = get_quadruple_mark_key(mark.r, mark.c);
            ...
            const symKey = get_quadruple_mark_key(symRow, symCol);
    ...
    function temporarily_remove_marks_state(keys = []) {
        ...
        for (const m of state.marks_board) {
            if (m && m.kind === 'x' && Number.isInteger(m.r) && Number.isInteger(m.c)) {
                const key = get_quadruple_mark_key(m.r, m.c);
                if (keySet.has(key)) {
                    removed.push(m);
                    continue;
                }
    ...
    function restore_marks_state(removedMarks) {
        for (const m of removedMarks || []) {
            ...
            upsert_quadruple_mark(m.r, m.c, clue);
        }
    }
}

function get_quadruple_mark_key(row, col) {
    return `x-${row + 1}-${col + 1}`;
}

function render_quadruple_marks_from_state(size, container = document.querySelector('.sudoku-container')) {
    if (!container) return;
    ...
    for (const markData of marks) {
        ...
        const key = get_quadruple_mark_key(row, col);
...
function add_quadruple_mark(size) {
    ...
    grid.addEventListener('click', function handler(e) {
        ...
        const key = get_quadruple_mark_key(markRow, markCol);
