...
export function solve_By_Elimination(board, size) {
    ...
    const techniqueGroups = [
        ...
        [
            ...
            () => state.techniqueSettings?.Row_Hidden_Pair && check_Row_Hidden_Subset_Elimination(board, size, 2),
            () => state.techniqueSettings?.Col_Hidden_Pair && check_Col_Hidden_Subset_Elimination(board, size, 2)
        ],
        // 第十六优先级：行列隐性三数组
        [
            ...
            () => state.techniqueSettings?.Row_Hidden_Triple && check_Row_Hidden_Subset_Elimination(board, size, 3),
            () => state.techniqueSettings?.Col_Hidden_Triple && check_Col_Hidden_Subset_Elimination(board, size, 3)
        ],
        // 第十七优先级：宫显性三数组
                    ...
                    if (!groupChanged && !isEqual(board, groupInitialBoard)) {
                        ...
                        if (techniqueName) {
                            ...
                            switch(techniqueName) {
                                ...
                                case 'Col_Elimination': chineseName = "行列排除"; break;
                                case 'Box_Block': chineseName = "宫区块"; break;
                                case 'Row_Block':
                                case 'Col_Block': chineseName = "行列区块"; break;
                                case 'Box_Naked_Pair': chineseName = "宫显性数对"; break;
                                case 'Row_Naked_Pair':
                                case 'Col_Naked_Pair': chineseName = "行列显性数对"; break;
                                case 'Box_Naked_Triple': chineseName = "宫显性三数组"; break;
                                case 'Row_Naked_Triple':
                                case 'Col_Naked_Triple': chineseName = "行列显性三数组"; break;
                                case 'Box_Naked_Quad': chineseName = "宫显性四数组"; break;
                                case 'Row_Naked_Quad':
                                case 'Col_Naked_Quad': chineseName = "行列显性四数组"; break;
                                case 'Box_Hidden_Pair': chineseName = "宫隐性数对"; break;
                                case 'Row_Hidden_Pair':
                                case 'Col_Hidden_Pair': chineseName = "行列隐性数对"; break;
                                case 'Box_Hidden_Triple': chineseName = "宫隐性三数组"; break;
                                case 'Row_Hidden_Triple':
                                case 'Col_Hidden_Triple': chineseName = "行列隐性三数组"; break;
                                case 'Box_Hidden_Quad': chineseName = "宫隐性四数组"; break;
                                case 'Row_Hidden_Quad':
                                case 'Col_Hidden_Quad': chineseName = "行列隐性四数组"; break;
            ...
            if (numPositions[num].length > 1) {
                ...
                if (allSameBox) {
                    ...
                    if (excludedCells.length > 0) {
                        ...
                        if (!state.silentMode) log_process(`[行区块排除] ${blockCells}构成${num}区块，排除${excludedCells.join('、')}的${num}`);
                        return;
                    }
                }
            }
            ...
            if (numPositions[num].length > 1) {
                ...
                if (allSameBox) {
                    ...
                    if (excludedCells.length > 0) {
                        ...
                        if (!state.silentMode) log_process(`[列区块排除] ${blockCells}构成${num}区块，排除${excludedCells.join('、')}的${num}`);
                        return;
                    }
                }
            }
        ...
        for (let num2 = num1 + 1; num2 <= size; num2++) {
          ...
          if (positions.length === 2) {
            ...
            if (isPair) {
              ...
              if (modified) {
                const cells = positions.map(([r, c]) => `${getRowLetter(r+1)}${c+1}`).join('、');
                if (!state.silentMode) log_process(`[宫隐性数对] ${cells}构成隐性数对${num1}${num2}，删除其他候选数`);
                return;
              }
            }
          }
                    ...
                    for (let num3 = num2 + 1; num3 <= size; num3++) {
                        ...
                        if (positions.length === 3) {
                            ...
                            if (isTriple) {
                                ...
                                if (modified) {
                                    const cells = positions.map(([r, c]) => `${getRowLetter(r+1)}${c+1}`).join('、');
                                    if (!state.silentMode) log_process(`[宫隐性三数组] ${cells}构成隐性三数组${num1}${num2}${num3}，删除其他候选数`);
                                    return;
                                }
                            }
                        }
                        ...
                        for (let num4 = num3 + 1; num4 <= size; num4++) {
                            ...
                            if (positions.length === 4) {
                                ...
                                if (isQuad) {
                                    ...
                                    if (modified) {
                                        const cells = positions.map(([r, c]) => `${getRowLetter(r+1)}${c+1}`).join('、');
                                        if (!state.silentMode) log_process(`[宫隐性四数组] ${cells}构成隐性四数组${num1}${num2}${num3}${num4}，删除其他候选数`);
                                        return;
                                    }
                                }
                            }
                ...
                if (positions.length === 2) {
                    ...
                    if (isPair) {
                        ...
                        if (modified) {
                            ...
                            if (!state.silentMode) log_process(`[行隐性数对] ${cells}构成隐性数对${num1}${num2}，删除其他候选数`);
                            return;
                        }
                    }
                }
                ...
                for (let num3 = num2 + 1; num3 <= size; num3++) {
                    ...
                    if (positions.length === 3) {
                        ...
                        if (isTriple) {
                            ...
                            if (modified) {
                                const cells = positions.map(col => `${getRowLetter(row+1)}${col+1}`).join('、');
                                if (!state.silentMode) log_process(`[行隐性三数组] ${cells}构成隐性三数组${num1}${num2}${num3}，删除其他候选数`);
                                return;
                            }
                        }
                    }
                    ...
                    for (let num4 = num3 + 1; num4 <= size; num4++) {
                        ...
                        if (positions.length === 4) {
                            ...
                            if (isQuad) {
                                ...
                                if (modified) {
                                    const cells = positions.map(col => `${getRowLetter(row+1)}${col+1}`).join('、');
                                    if (!state.silentMode) log_process(`[行隐性四数组] ${cells}构成隐性四数组${num1}${num2}${num3}${num4}，删除其他候选数`);
                                    return;
                                }
                            }
                        }
                ...
                if (positions.length === 2) {
                    ...
                    if (isPair) {
                        ...
                        if (modified) {
                            ...
                            if (!state.silentMode) log_process(`[列隐性数对] ${cells}构成隐性数对${num1}${num2}，删除其他候选数`);
                            return;
                        }
                    }
                }
                ...
                for (let num3 = num2 + 1; num3 <= size; num3++) {
                    ...
                    if (positions.length === 3) {
                        ...
                        if (isTriple) {
                            ...
                            if (modified) {
                                const cells = positions.map(row => `${getRowLetter(row+1)}${col+1}`).join('、');
                                if (!state.silentMode) log_process(`[列隐性三数组] ${cells}构成隐性三数组${num1}${num2}${num3}，删除其他候选数`);
                                return;
                            }
                        }
                    }
                    ...
                    for (let num4 = num3 + 1; num4 <= size; num4++) {
                        ...
                        if (positions.length === 4) {
                            ...
                            if (isQuad) {
                                ...
                                if (modified) {
                                    const cells = positions.map(row => `${getRowLetter(row+1)}${col+1}`).join('、');
                                    if (!state.silentMode) log_process(`[列隐性四数组] ${cells}构成隐性四数组${num1}${num2}${num3}${num4}，删除其他候选数`);
                                    return;
                                }
                            }
                        }
...
function check_Row_Hidden_Subset_Elimination(board, size, subsetSize = 2) {
    let hasConflict = false;
    ...
    for (let row = 0; row < size; row++) {
        ...
        for (const numGroup of numCombinations) {
            ...
            const positions = [];
            for (const col of candidateCells) {
                if (numGroup.some(n => board[row][col].includes(n))) {
                    positions.push(col);
                }
            }
            ...
            if (positions.length === subsetSize) {
                ...
                let isSubset = true;
                for (const col of positions) {
                    if (!numGroup.every(n => board[row][col].includes(n))) {
                        isSubset = false;
                        break;
                    }
                }
                ...
                if (isSubset) {
                    ...
                    for (const col of positions) {
                        const originalLength = board[row][col].length;
                        board[row][col] = board[row][col].filter(n => numGroup.includes(n));
                        if (board[row][col].length < originalLength) {
                            modified = true;
                        }
                    }
                    ...
                    if (modified) {
                        const subsetName = 
                            ...
                            subsetSize === 3 ? '三数组' : '四数组';
                        const cells = positions.map(col => `${getRowLetter(row+1)}${col+1}`).join('、');
                        if (!state.silentMode) log_process(`[行隐性${subsetName}] ${cells}构成隐性${subsetName}${numGroup.join('')}，删除其他候选数`);
                        return;
                    }
                }
            }
...
function check_Col_Hidden_Subset_Elimination(board, size, subsetSize = 2) {
    let hasConflict = false;
                ...
                if (isSubset) {
                    ...
                    if (modified) {
                        const subsetName = 
                            ...
                            subsetSize === 3 ? '三数组' : '四数组';
                        const cells = positions.map(row => `${getRowLetter(row+1)}${col+1}`).join('、');
                        if (!state.silentMode) log_process(`[列隐性${subsetName}] ${cells}构成隐性${subsetName}${numGroup.join('')}，删除其他候选数`);
                        return;
                    }
                }
            }
