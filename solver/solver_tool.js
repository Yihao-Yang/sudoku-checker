    ...
    switch (mode) {
        ...
        // }
                ...
                case 'add': {
                    ...
                    for (const m of marks) {
                        ...
                        if (m.kind === 'v') {
                            const cell_a = [m.r, m.c];
                            const cell_b = [m.r, m.c + 1];
                            if (cell_a[0] < 0 || cell_a[0] >= size || cell_a[1] < 0 || cell_a[1] >= size) continue;
                            if (cell_b[0] < 0 || cell_b[0] >= size || cell_b[1] < 0 || cell_b[1] >= size) continue;
                            ...
                            regions.push({ type: '特定组合', index, cells: [cell_a, cell_b], sum });
                            continue;
                        }
        
                        if (m.kind === 'h') {
                            const cell_a = [m.r, m.c];
                            const cell_b = [m.r + 1, m.c];
                            if (cell_a[0] < 0 || cell_a[0] >= size || cell_a[1] < 0 || cell_a[1] >= size) continue;
                            if (cell_b[0] < 0 || cell_b[0] >= size || cell_b[1] < 0 || cell_b[1] >= size) continue;
                        ...
                        if (m.kind === 'x') {
                            ...
                            if (cells.some(([rr, cc]) => rr < 0 || rr >= size || cc < 0 || cc >= size)) continue;
        
                            const index = cells
                                .slice()
                                .sort((a, b) => (a[0] - b[0]) || (a[1] - b[1]))
                                .map(([rr, cc]) => `${getRowLetter(rr + 1)}${cc + 1}`)
                                .join('-');
        ...
        case 'skyscraper': {
            ...
            for (let row = 1; row <= size; row++) {
                ...
                // 帮助：在当前 board 中查找该行是否已出现数字 size，返回第一个匹配的 colIndex（0-based），否则返回 -1
                const find_size_in_row = () => {
                    if (!board || !Array.isArray(board[row - 1])) return -1;
                    for (let c = 0; c < size; c++) {
                        if (board[row - 1][c] === size) return c;
                    }
                    return -1;
                };
        
                const pos = find_size_in_row();
                ...
                if (left_clue > 0) {
                    if (left_clue === 1) {
                        ...
                        if (pos !== -1) {
                            ...
                            const cells = [];
                            for (let c = 0; c <= pos; c++) cells.push([row - 1, c]);
                            const index = cells
                                .sort((a, b) => (a[0] - b[0]) || (a[1] - b[1]))
                                .map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`)
                                .join('-');
                            ...
                            const index = row_cells
                                ...
                                .map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`)
                                .join('-');
                            // const index = row_cells.map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`).join('-');
                ...
                if (right_clue > 0) {
                    if (right_clue === 1) {
                        ...
                        if (pos !== -1) {
                            ...
                            const cells = [];
                            for (let c = size - 1; c >= pos; c--) cells.push([row - 1, c]);
                            const index = cells
                                .sort((a, b) => (a[0] - b[0]) || (a[1] - b[1]))
                                .map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`)
                                .join('-');
                            // const index = cells.map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`).reverse().join('-');
                            regions.push({ type: '特定组合', index, cells });
                        } else {
                            // 回退：整行
                            const row_cells = [];
            ...
            for (let col = 1; col <= size; col++) {
                ...
                const find_size_in_col = () => {
                    for (let r = 0; r < size; r++) {
                        if (board && board[r] && board[r][col - 1] === size) return r;
                    }
                    return -1;
                };
        
                const pos = find_size_in_col();
                ...
                if (top_clue > 0) {
                    if (top_clue === 1) {
                        ...
                        if (pos !== -1) {
                            // 从外侧（上）最近的第一格 row=1 (index 0) 到包含 size 的格子 pos
                            const cells = [];
                            for (let r = 0; r <= pos; r++) cells.push([r, col - 1]);
                            const index = cells
                                .sort((a, b) => (a[0] - b[0]) || (a[1] - b[1]))
                                .map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`)
                                .join('-');
                            ...
                            const index = col_cells
                                ...
                                .map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`)
                                .join('-');
                            // const index = col_cells.map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`).join('-');
                ...
                if (bottom_clue > 0) {
                    if (bottom_clue === 1) {
                        ...
                        if (pos !== -1) {
                            // 从外侧（下）最近的第一格 row=size (index size-1) 向上到包含 size 的格子 pos
                            const cells = [];
                            for (let r = size - 1; r >= pos; r--) cells.push([r, col - 1]);
                            const index = cells
                                .sort((a, b) => (a[0] - b[0]) || (a[1] - b[1]))
                                .map(([r, c]) => `${getRowLetter(r + 1)}${c + 1}`)
                                .join('-');
...
export function sync_marks_board_from_dom(size, container = document.querySelector('.sudoku-container')) {
    ...
    const grid = container.querySelector('.sudoku-grid');
    const grid_offset_left = grid?.offsetLeft ?? 0;
    const grid_offset_top = grid?.offsetTop ?? 0;
    const cell_width = grid ? grid.offsetWidth / size : 0;
    ...
    for (const markEl of domMarks) {
        ...
        if (key?.startsWith('v-')) {
            ...
            const cBetween = parseInt(cStr, 10);
            const c = cBetween - 1; // 左格列号
            if (Number.isInteger(r) && Number.isInteger(c) && r >= 0 && r < size && c >= 0 && c < size - 1) {
                marks.push({ kind: 'v', r, c, sum: add });
            }
            continue;
        }
        ...
        if (key?.startsWith('h-')) {
            const [, rStr, cStr] = key.split('-');
            const rBetween = parseInt(rStr, 10);
            const c = parseInt(cStr, 10);
            const r = rBetween - 1; // 上格行号
            if (Number.isInteger(r) && Number.isInteger(c) && r >= 0 && r < size - 1 && c >= 0 && c < size) {
                marks.push({ kind: 'h', r, c, sum: add });
            }
            continue;
        }
        ...
        if (key?.startsWith('x-')) {
            ...
            const r = rMark - 1; // 2x2 左上
            const c = cMark - 1;
            if (Number.isInteger(r) && Number.isInteger(c) && r >= 0 && r < size - 1 && c >= 0 && c < size - 1) {
                marks.push({ kind: 'x', r, c, sum: add });
            }
            continue;
        }

        // 兜底：没有 key 时按像素反推交点（保持与旧逻辑兼容）
